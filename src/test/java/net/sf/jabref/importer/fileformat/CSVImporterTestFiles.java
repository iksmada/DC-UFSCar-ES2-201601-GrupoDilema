...
public class CSVImporterTestFiles {
    ...
    public static Collection<String> fileNames() throws IOException {
        ...
        return files.stream().filter(n -> n.startsWith("CSVImporterTest")).filter(n -> n.endsWith(".txt"))
                .collect(Collectors.toList());
    }

    @Test
    public void testIsRecognizedFormat() throws IOException {
        try (InputStream stream = CSVImporterTest.class.getResourceAsStream(fileName)) {
    ...
    public void testImportEntries() throws IOException {
        ...
        try (InputStream csvStream = CSVImporterTest.class.getResourceAsStream(fileName);
                InputStream bibStream = BibtexImporterTest.class.getResourceAsStream(bibFileName)) {
            BibEntryAssert.assertEquals(bibStream, csvStream, csvImporter);
        }
